...
echo '<div class="testimonials-style-3">';
    if(!empty($instance['bg_title'])){
        echo '<h3 class="bg-title">'.ent2ncr($instance['bg_title']).'</h3>';
    }
    if ( $lop_testimonial->have_posts() ) {
        $html .= '<div class="sc-testimonials container owl-carousel" data-column="'.esc_attr($number_column).'" data-time="' . esc_attr($instance['time']) . '">';
