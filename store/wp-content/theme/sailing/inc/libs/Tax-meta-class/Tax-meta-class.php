...
if ( ! class_exists( 'Tax_Meta_Class' ) ) :
     ...
     */
        ...
        <img src="';
					...
					echo '" alt="' . __( 'Remove', 'sailing' ) . '" title="' . __( 'Remove', 'sailing' ) . '" id="remove-' . $field['id'] . '"></div>';
					$c = $c + 1;
			...
			echo '" alt="' . __( 'Add', 'sailing' ) . '" title="' . __( 'Add', 'sailing' ) . '" id="add-' . $field['id'] . '"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
		...
		public function show_field_image( $field, $meta ) {
			...
			echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
			if ( $has_image ) {
				echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='" . __( 'Remove Image','sailing' ) . "' type='button'/>";
			} else {
				echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='" . __( 'Upload Image','sailing' ) . "' type='button'/>";
			}
			$this->show_field_end( $field, $meta );
		}
