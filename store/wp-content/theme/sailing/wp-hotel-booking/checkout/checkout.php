...
?>
	...
	<div id="hotel-booking-payment">
		...
		<form name="hb-payment-form" id="hb-payment-form" method="post" action="<?php echo isset( $search_page ) ? $search_page : ''; ?>">
			<h3><?php _e( 'Booking Rooms', 'sailing' ); ?></h3>
			<table class="hb_table">
				<thead>
				<th class="hb_room_type"><?php _e( 'Room type', 'sailing' ); ?></th>
						...
						<tr class="hb_payment_all">
							<td colspan="9" class="hb-align-right">
								<label class="hb-align-right">
									...
									<?php _e( 'I want to pay all', 'sailing' ); ?>
								</label>
							</td>
						</tr>
					<?php } ?>
			...
			<?php if ( !is_user_logged_in() && !hb_settings()->get( 'guest_checkout' ) && get_option( 'users_can_register' ) ) : ?>
				...
				<?php printf( __( 'You have to <strong><a href="%s">login</a></strong> or <strong><a href="%s">register</a></strong> to checkout.', 'sailing' ), wp_login_url( hb_get_checkout_url() ), wp_registration_url() ) ?>
				...
				<?php if ( $tos_page_id = hb_get_page_id( 'terms' ) ) { ?>
					<p>
						<label>
							...
							<?php printf( __( 'I agree with ', 'sailing' ) . '<a href="%s" target="_blank">%s</a>', get_permalink( $tos_page_id ), get_the_title( $tos_page_id ) ); ?>
						</label>
					</p>
				<?php } ?>
				<p>
					<button type="submit" class="hb_button"><?php _e( 'Check out', 'sailing' ); ?></button>
				</p>
