...
?>
<h3 class="book-title"><?php esc_html_e( 'Buy Ticket', 'sailing' ); ?></h3>
...
<div class="event_register_area">
	...
	<form name="event_register" class="event_register" method="POST">
		...
		<ul class="info-event">
			<li>
				<div class="label"><?php esc_html_e( 'Total Slots', 'sailing' ); ?></div>
				<div class="value"><?php echo absint( $event->qty ); ?></div>
			</li>
			<li>
				<div class="label"><?php esc_html_e( 'Booked Slots', 'sailing' ); ?></div>
				<div class="value"><?php echo esc_html( $event->booked_quantity() ); ?></div>
			</li>
			<li class="event-cost">
				<div class="label"><?php esc_html_e( 'Cost', 'sailing' ); ?></div>
				<div
					class="value"><?php echo ( $event->get_price() ) ? wpems_format_price( $event->get_price() ) . esc_html__( '/Slot', 'sailing' ) : '<span class="free">' . esc_html__( 'Free', 'sailing' ) . '</span>'; ?></div>
			</li>
			<li>

				<?php if ( $user_reg == 0 && $event->is_free() && wpems_get_option( 'email_register_times' ) === 'once' ) { ?>
					<div class="label"><?php esc_html_e( 'Quantity', 'sailing' ); ?></div>
					<div class="value">
			...
			<?php if ( ! $event->is_free() ) : ?>
				...
				if ( ! empty( $payments ) ) {
					...
					<li class="event-payment">
						<div class="label"><?php esc_html_e( 'Pay with', 'sailing' ); ?></div>
		...
		<?php if ( empty( $payments ) ) { ?>
			<p class="event_auth_register_message_error">
				<?php echo esc_html__( 'You must set payment setting!', 'sailing' ); ?>
			</p>
		<?php } ?>
		...
		<div class="event_register_foot">
			...
			<?php if ( $is_expired ) : ?>
				<button type="submit" disabled
						class="event_button_disable"><?php esc_html_e( 'Expired', 'sailing' ); ?></button>
			<?php elseif ( absint( $event->qty ) == 0 ) : ?>
				<button type="submit" disabled
						class="event_button_disable"><?php esc_html_e( 'Sold Out', 'sailing' ); ?></button>
			<?php else : ?>
				<?php if ( ! is_user_logged_in() ) { ?>
					<a href="<?php echo esc_url( add_query_arg( 'redirect_to', urlencode( get_permalink() ), get_permalink( get_option( 'thimpress_events_login_page_id' ) ) ) ) ?>"
					   class="event_register_submit event_auth_button js-show-popup"><?php esc_html_e( 'Login Now', 'sailing' ); ?></a>
					<p></p>
					<p class="login-notice">
						<?php echo esc_html__( 'You must login to our site to book this event!', 'sailing' ); ?>
					</p>
				<?php } else { ?>
					<button type="submit"
							class="event_register_submit event_auth_button" <?php echo $payments ? '' : 'disabled="disabled"' ?>><?php esc_html_e( 'Book Now', 'sailing' ); ?></button>
				<?php } ?>
			<?php endif ?>
