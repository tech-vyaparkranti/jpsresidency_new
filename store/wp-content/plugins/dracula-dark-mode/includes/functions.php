...
function dracula_get_config()
{
    ...
    $background_color = dracula_get_settings( 'backgroundColor', '#181a1b' );
    $text_color = dracula_get_settings( 'textColor', '#e8e6e3' );
...
function dracula_reading_mode_get_button_html( $post_id )
{
    ...
    if ( !empty($icon) ) {
        $icon = sprintf( '<span class="reading-mode-icon-wrap">%s</span>', $icon );
    }
