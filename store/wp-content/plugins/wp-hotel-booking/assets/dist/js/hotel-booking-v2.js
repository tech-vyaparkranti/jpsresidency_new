...
const requestSearchRoom = function (forms, args) {
  let btn = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
...
const bookingRoomsPages = formsCheck => {
  ...
  const submit = async function (form) {
    let btn = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    let numRoom = arguments.length > 2 ? arguments[2] : undefined;
    let roomID = arguments.length > 3 ? arguments[3] : undefined;
    const extraData = [];
