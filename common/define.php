...
// ✅ Define your DOCBASE here — adjust if your folder name is different
// if (!defined('DOCBASE')) define('DOCBASE', '/jpsresidency_new/');
...
if (is_file(SYSBASE . 'common/config.php')) {
    ...
    try {
        ...
        if (ADMIN) $_SESSION['msg_error'][] = $pms_texts['DATABASE_ERROR'] ?? 'Database connection error.';
...
if ($result_currency !== false) {
    foreach ($result_currency as $row) {
...
// ✅ Load languages
$result_lang = $pms_db->query("SELECT l.id AS lang_id, lf.id AS file_id, title, tag, file, locale, rtl, main
    FROM pm_lang as l, pm_lang_file as lf
    WHERE id_item = l.id AND l.checked = 1 AND file != ''
    ORDER BY l.rank");
if ($result_lang !== false) {
    foreach ($result_lang as $row) {
        $lang_tag = $row['tag'];
        if ($row['main'] == 1) {
...
if (!ADMIN && (PMS_MAINTENANCE_MODE == 0 || (isset($_SESSION['user']) && in_array($_SESSION['user']['type'], ['administrator', 'manager'])))) {
    ...
    foreach ($result_text as $row) $pms_texts[$row['name']] = $row['value'];

    // Pages
    $pms_pages = [];
    $pms_sys_pages = [];
    $pms_parents = [];
    $result_page = $pms_db->query("SELECT * FROM pm_page
        WHERE (checked = 1 OR checked = 0)
        AND lang = $id_lang
        AND (show_langs IS NULL OR show_langs = '' OR show_langs REGEXP '(^|,)$id_lang(,|$)')
        AND (hide_langs IS NULL OR hide_langs = '' OR hide_langs NOT REGEXP '(^|,)$id_lang(,|$)')
        ORDER BY `rank`");
    ...
    // Articles
    $articles = [];
    $result_article = $pms_db->query("SELECT * FROM pm_article
    ...
    if ($result_menu !== false) {
        foreach ($result_menu as $row) {
            if (
                ($row['item_type'] == 'page' && isset($pms_pages[$row['id_item']]) && $pms_pages[$row['id_item']]['checked'] == 1) ||
                ...
                $row['href'] = $href;
                $row['target'] = (strpos($href, 'http') !== false) ? '_blank' : '_self';
...
// Currency config
$currency_code = $_SESSION['currency']['code'] ?? $default_currency_code;
$currency_sign = $_SESSION['currency']['sign'] ?? $default_currency_sign;
$currency_rate = $_SESSION['currency']['rate'] ?? $default_currency_rate;
