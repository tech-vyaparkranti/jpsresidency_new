        ...
        && is_numeric($_POST['adults_'.$activity_id]) && is_numeric($_POST['children_'.$activity_id]) && is_numeric($_POST['date_'.$activity_id])){
            ...
            $result_book = $db->query('
                            ...
                            WHERE
                                ...
                                AND id_booking = b.id
                                AND id_activity = a.id
                                AND (status = 4 OR (status = 1 AND (add_date > '.(time()-900).' OR payment_option IN(\'On arrival\',\'Check\'))))
            ...
            if(!empty($sessions) && $amount > 0){
                ...
                <input type="hidden" name="amount_'.$activity_id.'" value="'.number_format($amount, 10, '.', '').'">
                <input type="hidden" name="duty_free_'.$activity_id.'" value="'.number_format($duty_free, 10, '.', '').'">';
                foreach($taxes as $tax_id => $tax_amount)
                    $response['html'] .= '<input type="hidden" name="taxes_'.$activity_id.'['.$tax_id.']" value="'.number_format($tax_amount, 10, '.', '').'">';
            }
        }
    }
}
